    ...
    <div style="margin:0 auto;padding:20px;background:white; color: #999999;">
        ...
        <div>
            ...
            <div>Hi {{ $user->get_fullname() }},<br>
                <p> Your payment request for {{ $paymentData->amount }} to {{ $shopName }} store has been accepted</p>
                <br>

                @if($description != '')
                <hr style="border-color: #EEEEEE; border-style: solid;">
                <p>Your description was below :</p>

                <p>{{ nl2br($description) }}</p>
                <hr style="border-color: #EEEEEE; border-style: solid;">
                @endif

                <p>Your payment method was <strong>{{ ucfirst(str_replace('_', ' ', $paymentData->payment_way)) }}</strong></p>
